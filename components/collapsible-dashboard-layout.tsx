    ...
    <div className="flex h-screen overflow-hidden bg-background w-full">
      <aside
        className={cn(
          ...
          isProjectPage && isCollapsed ? "w-0 opacity-0 border-r-0 pointer-events-none" : "w-64 opacity-100"
        )}
      >
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden">
          ...
          <nav className="p-4 pt-8 space-y-1">
            ...
            <SidebarNav role={role} hasUnit={!!userTeam} />
          </nav>
        </div>
      ...
      {isProjectPage && isCollapsed && mounted && (
        <button
          onClick={handleToggle}
          className="fixed left-0 top-1/2 -translate-y-1/2 z-30 flex h-16 w-4 items-center justify-center rounded-r-md border border-l-0 border-border bg-card shadow-sm cursor-pointer hover:bg-muted text-muted-foreground hover:text-foreground transition-all duration-200 outline-none animate-in fade-in duration-300"
